...
#define list_entry(ptr, type, member)					\
	((type *)((char *)(ptr)-(unsigned long)(&((type *)0)->list)))
...
#define list_for_each_entry(pos, head, member)				\
	for (pos = list_entry((head)->next, IDNode_t, member);	\
	     &pos->member != (head); 					\
	     pos = list_entry(pos->member.next, IDNode_t, member))
...
#define list_for_each_entry_safe(pos, n, head, member)			\
	for (pos = list_entry((head)->next, IDNode_t, member),	\
		n = list_entry(pos->member.next, IDNode_t, member);	\
	     &pos->member != (head); 					\
	     pos = n, n = list_entry(n->member.next, IDNode_t, member))
