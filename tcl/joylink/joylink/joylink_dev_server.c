...
joylink_server_st_init()
{
   ...
   // saServer.sin_family = AF_INET;
    ...
    saServer.s_port = (_g_pdev->jlp.server_port);
    ...
    saServer.s_ip = *((struct in_addr *)host->h_addr);
#else
   saServer.s_ip = inet_addr(_g_pdev->jlp.joylink_server);
#endif
    log_error("jorlink_ip==%s,%d",_g_pdev->jlp.joylink_server,saServer.s_port);
  ...
  // bind(_g_pdev->server_socket, &saServer, sizeof(saServer));
...
joylink_server_st_auth()
{
    ...
    for(i=0;i<len;i++)
    	{
printf("%02x ",_g_pdev->send_buff[i]);
    	}
    	log_debug("\r\n认证结束");
    if(ret < 0){
...
joylink_proc_server_st()
{
	...
	switch (_g_pdev->server_st){
        ...
        //    interval = 500;//wcz del
            interval = 5000;//wcz add
            _g_pdev->hb_lost_count++;
            break;
...
joylink_proc_server_auth(uint8_t* recPainText)
{
    ...
    set_Tcl_UTC(p->timestamp);//wcz wcz_add
...
joylink_proc_server_ctrl(uint8_t* recPainText)
{
    ...
    log_debug("Control from server:%s:len:%d\n",
            recPainText , strlen((char*)(recPainText )));

    log_debug("Control from server:%s:len:%d\n",
            recPainText + 12, strlen((char*)(recPainText + 12)));

    if(-1 == joylink_dev_lan_script_ctrl((const char *)recPainText, &ctr, 2)){
        return;
    }
    ret = joylink_packet_lan_script_ctrl_rsp(data, sizeof(data), &ctr);
