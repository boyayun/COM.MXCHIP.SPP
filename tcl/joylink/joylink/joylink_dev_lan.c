...
joylink_proc_lan_script_ctrl(uint8_t *src, struct sockaddr_in *sin_recv, socklen_t addrlen)
{
    ...
    if(-1 == joylink_dev_lan_script_ctrl((const char *)src, &ctr, 0)){
        return;
    }
...
joylink_proc_lan()
{
    ...
    socklen_t addrlen = sizeof(sin_recv);

    ret = recvfrom(_g_pdev->lan_socket, recBuffer, 
            sizeof(recBuffer), 0, &sin_recv, &addrlen);

    if(ret == -1){
        goto RET;
    }
    if(*(unsigned int *)recBuffer==0x123455bb)
    	{
     log_info("joylink:rec:start\r\n");
		    for(i=0;i<ret;i++)
		    	{
    ...
    switch (param.type){
        ...
        case PT_SCRIPTCONTROL:
            if(param.version == 1){
                log_debug("Control->%s", recPainText + 12);
